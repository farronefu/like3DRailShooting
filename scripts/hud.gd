...
const CYAN := Color("c9dcc1")
const WHITE := Color("e8f0f5")
const MUTED := Color("a4b3ab")
...
func draw_menu() -> void:
	...
	if game.state == "menu":
		...
		label_at("01  /  THE EMERALD RIVER", Vector2(68, 346), 19, CYAN)
		label_at("Follow the river through a defended forest valley.", Vector2(68, 384), 18)
		label_at("Survive the approach. Destroy the River Guardian.", Vector2(68, 414), 17, MUTED)
		draw_controls(458)
		...
		label_at("River Guardian destroyed. The valley is secure." if cleared else "Your wing was lost. The river awaits another run.", Vector2(68, 265), 18, MUTED)
	...
	label_at("FOREST RUN + BOSS", Vector2(933, 535), 19)
	label_at("03:00 APPROACH  /  DEFEAT THE BOSS", Vector2(933, 568), 12, MUTED)

func draw_controls(y: float) -> void:
	label_at("MOUSE + KEYBOARD", Vector2(68, y), 13, CYAN)
	label_at("WASD  Move    Mouse  Aim    Click / Space  Fire", Vector2(68, y + 27), 16)
	label_at("GAMEPAD", Vector2(68, y + 63), 13, CYAN)
	label_at("L stick  Move    R stick  Aim    A / RT  Fire", Vector2(68, y + 90), 16)
...
func draw_flight() -> void:
	...
	label_at("%s  /  %s" % [format_time(game.elapsed), "BOSS" if game.elapsed >= game.rules.BOSS_ARRIVAL else "03:00"], Vector2(501, 82), 15)
	draw_rect(Rect2(480, 99, 320 * minf(game.elapsed / game.rules.BOSS_ARRIVAL, 1.0), 3), CYAN)
	panel(Rect2(1010, 30, 173, 73))
	label_at("SCORE", Vector2(1028, 55), 11, MUTED)
	label_at("%06d" % game.score, Vector2(1028, 84), 27)
	var reticle: Vector2 = game.aim_screen * Vector2(1280, 720)
	var r := Rect2(reticle - Vector2(19, 19), Vector2(38, 38))
	for side in [-1.0, 1.0]:
		for vertical in [-1.0, 1.0]:
			var corner := reticle + Vector2(side, vertical) * 19
			draw_line(corner, corner - Vector2(side * 9, 0), Color(0.02,0.04,0.03,0.85), 5)
			draw_line(corner, corner - Vector2(0, vertical * 9), Color(0.02,0.04,0.03,0.85), 5)
			draw_line(corner, corner - Vector2(side * 9, 0), CYAN, 2)
			draw_line(corner, corner - Vector2(0, vertical * 9), CYAN, 2)
	draw_circle(r.get_center(), 4, Color("132219"))
	...
	if is_instance_valid(game.boss):
		panel(Rect2(390, 560, 500, 56), 0.84)
		label_at("RIVER GUARDIAN  /  " + ("ENRAGED" if game.boss.phase == 2 else "ARMORED"), Vector2(410,582), 13, ORANGE)
		draw_rect(Rect2(410,596,460,6),Color("34483b"))
		draw_rect(Rect2(410,596,460*game.boss.health/game.boss.MAX_HEALTH,6),ORANGE)
		if game.boss.telegraph:
			label_at("VOLLEY INCOMING — KEEP MOVING",Vector2(459,535),15,ORANGE)
			for side in [-1.0,1.0]:
				var muzzle: Vector3 = game.boss.position+Vector3(side*7,-1.5,8)
				var marker: Vector2 = game.camera.unproject_position(muzzle) / (size / Vector2(1280,720))
				draw_arc(marker,14,0,TAU,24,ORANGE,2,true)
	if game.banner_time > 0:
		panel(Rect2(320, 140, 640, 64), 0.8)
		label_at(game.banner, Vector2(343, 179), 18, CYAN)
	...
	label_at("WASD / L STICK  MOVE", Vector2(984, 662), 12, MUTED)
	label_at("MOUSE / R STICK  AIM", Vector2(1023, 684), 11, MUTED)
	if game.health <= 30:
